...
class LocationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Location entity")
        ...
        .repeat(2) {
            ...
            .get("/delivery/api/locations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new location")
            .post("/delivery/api/locations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "streetAddress":"SAMPLE_TEXT", "numberAddress":"0", "neighborhood":"SAMPLE_TEXT", "postalCode":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_location_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created location")
                .get("/delivery${new_location_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created location")
            .delete("/delivery${new_location_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
